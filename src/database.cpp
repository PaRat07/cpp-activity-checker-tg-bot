...
Database::Database()
    : db([] -> SQLite::Database {
        ...
        sqlite3_trace_v2(db.getHandle(),
                         SQLITE_TRACE_STMT,  // trace every statement
                         &traceCallback, nullptr);
#endif
        db.exec(
            "CREATE TABLE IF NOT EXISTS activity (activity_check_id INTEGER, participant_id INTEGER, "
            "UNIQUE(activity_check_id, participant_id))");
        db.exec(
            "CREATE TABLE IF NOT EXISTS activity_check (id INTEGER PRIMARY KEY AUTOINCREMENT, owner "
            "INTEGER)");
        return db;
      }()),
...
std::expected<std::filesystem::path, std::string_view> Database::GetActivityCheckList(
    int64_t check_id, tgbm::api::Integer asker) const {
  ...
  if (auto real_owner_opt = GetQueryOne<int>(get_owner_by_id); real_owner_opt.has_value()) {
    ...
    TGBM_LOG_DEBUG("access error: {} tried to access non-existing activity-check", asker);
    return std::unexpected("ОШИБКА: данной проверки активности не существует.");
  }

  if (real_owner != asker.value) {
    TGBM_LOG_DEBUG("access error: {} tried to access {} activity check(owner: {})", asker, check_id,
                   real_owner);
...
std::string_view Database::AddActivity(int64_t check_id, int64_t userId, std::string_view fullName,
...
int64_t Database::AddActivityCheck(int64_t owner) {
